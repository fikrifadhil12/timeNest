...
class TaskDB {
  ...
  /// Mengambil statistik tugas (completed dan pending)
  Future<Map<String, int>> getTaskStatistics() async {
    var db = await _appDatabase.getDb();
    var result = await db.rawQuery(
        'SELECT ${Tasks.dbStatus}, COUNT(*) as count FROM ${Tasks.tblTask} GROUP BY ${Tasks.dbStatus}');
    
    int completed = 0;
    int pending = 0;

    for (var row in result) {
      if (row[Tasks.dbStatus] == TaskStatus.COMPLETE.index) {
        completed = row['count'] as int;
      } else if (row[Tasks.dbStatus] == TaskStatus.PENDING.index) {
        pending = row['count'] as int;
      }
    }
